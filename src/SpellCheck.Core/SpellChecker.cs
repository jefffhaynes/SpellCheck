...
namespace SpellCheck.Core
{
    ...
    public class SpellChecker : IDisposable
    {
        ...
        public static IEnumerable<string> SupportedLanguages
        {
            ...
            {
                var factory = new SpellCheckerFactory();
                IEnumString langs = null;

                try
                {
                    langs = factory.SupportedLanguages;

                    langs.RemoteNext(1, out var currentLang, out _);

                    while (currentLang != null)
                    {
                        yield return currentLang;
                        langs.RemoteNext(1, out currentLang, out _);
                    }
                }
                finally
                {
        ...
        /// </summary>
        public string LanguageTag => _spellChecker.languageTag;
        ...
        public IEnumerable<string> Suggestions(string word)
        {
            ...
            try
            {
                suggestions.RemoteNext(1, out var currentSuggestion, out _);

                while (currentSuggestion != null)
                {
                    yield return currentSuggestion;
                    suggestions.RemoteNext(1, out currentSuggestion, out _);
                }
            }
            finally
            {
        ...
        public IEnumerable<SpellingError> Check(string text)
        {
            ...
            try
            {
                while ((currentError = errors.Next()) != null)
                {
                    ...
                    {
                        case CorrectiveAction.CorrectiveActionDelete:
                            action = RecommendedAction.Delete;
                            break;

                        case CorrectiveAction.CorrectiveActionGetSuggestions:
                            action = RecommendedAction.GetSuggestions;
                            break;

                        case CorrectiveAction.CorrectiveActionReplace:
        ...
        {
            if (_spellChecker is ISpellChecker2 spellChecker2)
            {
